package ru.nand.analyticsservice.utils;
...
public class JwtUtil {
    ...
    public int extractUserId(String token) {
        try {
            ...
            // Получаем значение user_id
            Integer userId = claims.get("user_id", Integer.class);

            if (userId == null) {
                throw new RuntimeException("Токен не содержит user_id");
            }

            return userId;
        } catch (Exception e) {
            log.error("Ошибка при извлечении user_id из токена: {}", e.getMessage());
            throw new RuntimeException("Ошибка извлечения user_id: " + e.getMessage());
        }
    }
}